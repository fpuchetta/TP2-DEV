...
{
	printf(ANSI_COLOR_GREEN "(" ANSI_COLOR_BOLD ANSI_COLOR_YELLOW
...
static bool agregar_accion_o_fallar(menu_t *menu_raiz, menu_t *menu_destino,
				    char tecla, const char *texto,
				    bool (*accion)(void *))
{
...
{
	if (!agregar_accion_o_fallar(menu_principal, menu_principal, 'C',
				     "Cargar archivo", accion_cargar_archivo))
		return false;

	if (!agregar_accion_o_fallar(menu_principal, menu_principal, 'J',
				     "Jugar", accion_jugar))
		return false;

	if (!agregar_accion_o_fallar(menu_principal, menu_principal, 'S',
				     "Jugar con semilla",
				     accion_jugar_con_semilla))
		return false;
...
menu_t *crear_submenu_buscar(menu_t *menu_principal)
{
	...
	if (!agregar_accion_o_fallar(menu_principal, menu_buscar, 'N',
				     "Buscar por nombre",
				     accion_buscar_por_nombre))
		return NULL;

	if (!agregar_accion_o_fallar(menu_principal, menu_buscar, 'I',
				     "Buscar por ID", accion_buscar_por_id))
		return NULL;
...
{
	menu_t *menu_mostrar =
		menu_crear_submenu(menu_principal, 'M', "Mostrar");
	if (!menu_mostrar) {
		menu_destruir_todo(menu_principal);
		return NULL;
	}

	if (!agregar_accion_o_fallar(menu_principal, menu_mostrar, 'N',
				     "Mostrar por nombre",
				     accion_mostrar_por_nombre))
		return NULL;

	if (!agregar_accion_o_fallar(menu_principal, menu_mostrar, 'I',
				     "Mostrar por ID", accion_mostrar_por_id))
		return NULL;
...
{
	menu_t *menu_principal = menu_crear_base("Menu principal TP2 - Pokedex",
						 estilo_cyber_panel,
						 estilo_titulo_pokemon);
	if (!menu_principal)
		return NULL;
...
bool ejecutar_menu_interactivo(menu_t *menu, juego_t *juego)
{
	...
	while (!menu_navegador_esta_terminado(nav)) {
		...
		menu_navegacion_estado_t estado =
			menu_navegador_procesar_tecla(nav, tecla);
...
tp2_t *tp2_ejecutar(tp2_t *tp2)
{
	...
	if (!ejecutar_menu_interactivo(tp2->menu_tp2, tp2->juego))
		return NULL;
