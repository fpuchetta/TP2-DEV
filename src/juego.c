...
bool callback_mostrar_movimientos(void *elemento, void *contexto)
{
	...
	if (!ctx->es_historial_personal &&
	    ctx->contador_actual >= ctx->maximo_a_mostrar) {
		return false;
	}
	...
	if (ctx->es_historial_personal) {
		printf("%s%zu)%s %d-%d%s%c%s\n", ANSI_COLOR_WHITE,
		       ctx->turno_actual, ANSI_COLOR_RESET, c1, c2, color_mov,
		       letra, ANSI_COLOR_RESET);
		ctx->turno_actual++;
	} else {
		const char *color_jugador =
			(mov->jugador == 0) ? COLOR_JUGADOR_1 : COLOR_JUGADOR_2;
		printf("%sJugador %d%s: %d-%d %s%c%s\n", color_jugador,
		       mov->jugador + 1, ANSI_COLOR_RESET, c1, c2, color_mov,
		       letra, ANSI_COLOR_RESET);
		ctx->contador_actual++;
	}
...
bool llenar_celdas(tp1_t *pokedex, carta_t *celdas)
{
	...
	if (total == 0) {
		printf("Error: No hay pokemones disponibles\n");
		return false;
	}
	...
	if (n < PAREJAS) {
		printf("Advertencia: Solo hay %zu pokemones únicos, se repetirán algunos\n",
		       total);
		while (n < PAREJAS) {
			struct pokemon *p = tp1_random(pokedex, total);
			elegidos[n] = p;
...
juego_t *juego_crear(const char *archivo)
{
	...
	if (archivo) {
		...
		if (tp1_cantidad(juego->pokedex) < 1) {
...
void mostrar_carta_descubierta(int indice_jugador_descubierto,
			       char nombre_abreviado[4])
{
	const char *color = (indice_jugador_descubierto == 0) ?
				    COLOR_JUGADOR_1 :
				    COLOR_JUGADOR_2;
	printf("%s[%s %3s %s]%s ", color, ANSI_COLOR_RESET, nombre_abreviado,
	       color, ANSI_COLOR_RESET);
}

void mostrar_carta_visible(char nombre_abreviado[4])
{
	printf(COLOR_SELECCIONADO "[" ANSI_COLOR_RESET
				  " %3s " COLOR_SELECCIONADO
				  "]" ANSI_COLOR_RESET " ",
	       nombre_abreviado);
}
	...
	for (size_t fila = 0; fila < mapa->filas; fila++) {
		for (size_t col = 0; col < mapa->columnas; col++) {
			...
			if (!carta->pokemon) {
				...
				char nombre_abreviado[4];
				strncpy(nombre_abreviado,
					carta->pokemon->nombre, 3);
				nombre_abreviado[3] = '\0';

				if (carta->encontrada) {
					mostrar_carta_descubierta(
						carta->jugador_descubridor,
						nombre_abreviado);
...
/*
    ...
    Post: Reinicia el juego al estado inicial con la semilla especificada.
          Devuelve true si se reinicio correctamente, false en caso de error.
*/
...
{
	return juego->cuadrilla->celdas[idx1].pokemon->id ==
	       juego->cuadrilla->celdas[idx2].pokemon->id;
}
...
movimiento_t *crear_movimiento(int jugador, int carta1, int carta2,
			       bool acierto)
{
...
bool agregar_jugada_historial(juego_t *juego, int idx1, int idx2, bool acierto)
{
	...
	movimiento_t *mov =
		crear_movimiento(juego->jugador_actual, idx1, idx2, acierto);
	if (!mov)
		return false;
	...
	lista_t *hist_personal_act =
		juego->jugadores[juego->jugador_actual].historial_personal;
...
void mostrar_ultimas_jugadas(juego_t *juego)
{
	if (!juego || !juego->historial_jugadas)
		return;
	if (lista_vacia(juego->historial_jugadas))
		return;
	...
	contexto_movimientos_t contexto = { .turno_actual = 0,
					    .numero_jugador = -1,
	...
	lista_con_cada_elemento(juego->historial_jugadas,
				callback_mostrar_movimientos, &contexto);
}
...
estado_jugada_t juego_validar_jugada(juego_t *juego, int carta1, int carta2)
{
	...
	if (carta1 < 0 || carta1 >= TOTAL_CARTAS || carta2 < 0 ||
	    carta2 >= TOTAL_CARTAS)
...
estado_jugada_t juego_ejecutar_jugada(juego_t *juego, int carta1, int carta2)
{
	...
	bool historial_ok =
		agregar_jugada_historial(juego, carta1, carta2, acierto);
	if (!historial_ok)
		return JUGADA_ERROR_MEMORIA;
	...
	if (acierto) {
		...
		juego->cuadrilla->celdas[carta1].jugador_descubridor =
			juego->jugador_actual;
		juego->cuadrilla->celdas[carta2].jugador_descubridor =
			juego->jugador_actual;
		juego->jugadores[juego->jugador_actual].puntaje++;
	} else {
		juego->jugador_actual =
			cambiar_indice_jugador(juego->jugador_actual);
			 ...
			 int numero_jugador)
{
	...
	contexto_movimientos_t contexto = { .turno_actual = 1,
					    .numero_jugador = numero_jugador,
					    .es_historial_personal = true,
					    .maximo_a_mostrar = 0,
					    .contador_actual = 0 };

	lista_con_cada_elemento(historial, callback_mostrar_movimientos,
				&contexto);
	printf("\n");
}
...
void mostrar_resultado_final(juego_t *juego)
{
	...
	printf("%sJugador 1:%s %s%d puntos%s\n", COLOR_JUGADOR_1,
	       ANSI_COLOR_RESET, ANSI_COLOR_BOLD, puntaje_j1, ANSI_COLOR_RESET);

	printf("%sJugador 2:%s %s%d puntos%s\n\n", COLOR_JUGADOR_2,
	       ANSI_COLOR_RESET, ANSI_COLOR_BOLD, puntaje_j2, ANSI_COLOR_RESET);

	mostrar_movimientos("🎯 HISTORIAL JUGADOR 1 🎯",
			    juego->jugadores[0].historial_personal, 0);
	mostrar_movimientos("🎯 HISTORIAL JUGADOR 2 🎯",
			    juego->jugadores[1].historial_personal, 1);

	printf("%s%s──────────────────────────────%s\n\n", ANSI_COLOR_WHITE,
	       ANSI_COLOR_BOLD, ANSI_COLOR_RESET);
	if (puntaje_j1 > puntaje_j2)
		printf("%s%s%s🎉 ¡JUGADOR 1 GANA! 🎉%s\n", ANSI_BG_CYAN,
		       ANSI_COLOR_BLACK, ANSI_COLOR_BOLD, ANSI_COLOR_RESET);
	else
...
{
	if (!juego || !juego_esta_preparado(juego) || jugador < 0 ||
	    jugador > 1)
		return -1;
...
{
	if (!juego || !juego_esta_preparado(juego) || indice_carta < 0 ||
	    indice_carta >= TOTAL_CARTAS)
		return false;
